...
const BitcoinTerminalDashboard = ({ currentPrice: sharedCurrentPrice, onPriceUpdate }) => {
  ...
  const fetchHistoricalData = useCallback(async (timeframe) => {
    // Don't fetch if we already have data for this timeframe and it's recent
    if (historicalData.length > 0 && selectedTimeframe === timeframe && 
        lastUpdate && (new Date() - lastUpdate) < 300000) { // 5 minutes
      console.log(`📊 Using recent ${timeframe} data, skipping fetch`);
      return;
    }
  ...
  const handleTimeframeChange = (timeframe) => {
    if (timeframe === selectedTimeframe) {
      console.log(`📊 Already on ${timeframe}, skipping fetch`);
      return;
    }
    
    console.log(`🔄 Changing timeframe from ${selectedTimeframe} to: ${timeframe}`);
    setSelectedTimeframe(timeframe);
    // fetchHistoricalData will be called by useEffect when selectedTimeframe changes
  };
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="fixed bottom-6 right-6 flex flex-col space-y-2">
        <button 
          onClick={() => {
            console.log('🔄 Manual portfolio refresh');
            fetchPortfolio();
          }}
